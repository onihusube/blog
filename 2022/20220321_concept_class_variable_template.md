...
先程の例では、プライマリテンプレートに制約を行わず、部分特殊化をわざわざ定義してからそちらに対してのみ制約を行っていました。なぜ？？
...
このことは直感的にも、プライマリテンプレートの`T`が`std::integral`を要求するのに、部分特殊化の対応する`T`が無制約なのは特殊化になっていなくない？と気づくことができるかもしれません。
...
とはいえ、コンセプトの順序を適切に判断するのも難しいものがあるので、プライマリテンプレートには制約を行わないことを推奨します。
