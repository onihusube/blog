...
プロジェクションを取るアルゴリズムについて、その制約を修正する提案。

プロジェクションを取るアルゴリズムでは、次のようなコードが動くように制約されているはずで、それが期待されます
...
これは、間接呼び出しに関するコンセプトとプロジェクションの合成によって制約を行なっているために起きています。例えば、[`std::ranges::for_each`](https://cpprefjp.github.io/reference/algorithm/ranges_for_each.html)の制約にある[`std::indirect_unary_invocable<std::projected<I,P>>`](https://cpprefjp.github.io/reference/iterator/indirectly_unary_invocable.html)がそれにあたります。

[`std::projected<I,P>`](https://cpprefjp.github.io/reference/iterator/projected.html)はイテレータ型`I`の間接参照結果（`*i`）をプロジェクション`P`に通した結果を（イテレータlikeな型として）返します。その型を`Prj`として、`indirect_unary_invocable<Pred, Prj>`は`Prj`の間接参照結果（`*prj`）を述語（`Pred`）に渡して呼び出し可能かがチェックされます。

アルゴリズム内で述語は、入力イテレータの要素への参照を渡して呼ばれることもあれば、それをコピーした値を渡して呼ばれることもあり、`std::ranges::unique_copy`のように両方を行うものもあります。

`indirectly_unary_invocable`などのコンセプトでは、その両方の呼び出し及び共通参照（*common reference*）に束縛してから呼び出しが可能であることをチェックする制約式が含まれています。
...
プロジェクションは関数アダプタであり、アルゴリズムの主たる振る舞いに影響を与えるものではなく、ユーザーは述語とプロジェクションを事前に関数合成したものを述語として渡したとしても分けて渡された時と同じ振る舞いをするはずです。実際そのように実装されているのですが、現在のイテレータとプロジェクション及び述語に課せられた制約はそのことを正しく表現できていません。例えば先ほどの`std::ranges::for_each`に対する`std::indirect_unary_invocable<std::projected<I,P>>`がそうで、その中の`iter_value_t`による呼び出しに関する制約式が問題を起こしています。
...
この制約は、`f(proj(*it))`が有効であることを誓約していますが、イテレータの要素をコピーしてから渡す場合は次のような違いがあります
...
すなわち、最初に（射影する前に）イテレータの要素をコピーしてからそれをプロジェクションと述語にかけたいはずなのに、`std::indirect_unary_invocable<std::projected<I,P>>`という制約は射影後の値をコピーしてそれを述語にかけるコードを許可するような制約になっています。
...
この提案はこれを解決するために、間接呼び出し系のコンセプトを修正しようとするものです。
