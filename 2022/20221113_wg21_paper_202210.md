...
1. 過剰なボイラープレートコード
    - アロケータをやり取りするためのコンストラクタと代入演算子を書かなければならない
...
スコープ付きアロケータモデルにおいて、内包するオブジェクトにまでアロケータが伝播していることは不変条件の一部であり、これに違反すると未定義動作になります。ただし、これを管理するのはかなり大変で、挿入するオブジェクトや大本オブジェクト自体のコピーが起こると、それは容易に破られます。

このような問題の解決はライブラリレベルではもはや不可能なため、言語機能の拡張によって解決しようとする提案です。

この提案では、`using`によってオブジェクト初期化時にスコープ付きの値（ここではアロケータ）を渡す構文を提案しています。
