...
*stride*を伴うレイアウトは`std::layout_stride`というクラスを`std::mdspan`に指定することで適切にハンドルでき、`std::layout_stride`は次元ごとに*stride*を指定できるため*padding stride*にも対応可能です。しかし、*padding stride*の場合は何処か1つの次元だけが*stride*を持つ、あるいは何処か1つの次元は*stride*の考慮が必要ないことがわかっており、`std::layout_stride`では非効率である可能性があります。
