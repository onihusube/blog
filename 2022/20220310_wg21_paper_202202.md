...
P2473R1で提案されているモジュール配布について、別のアプローチについての提案。

この提案のシナリオは次のようなものです

1. あるグループAはC++モジュールとしてライブラリAを構築し、BMI（*Binary Module Interface*）として配布したい。Aはそのパッケージに含まれていない別のモジュールに依存している。
...
この時、2と3においてビルド済みモジュールを活用するために必要な情報は何で、ライブラリAから何が必要で、どう見つかる必要があるか？を考え、それらの必要な情報提供の経路をJSONファイルとして提案するものです。

ビルド済みのモジュールを使用するには、以下の情報が必要です
...
シナリオ3.1、メインビルドが完了した後にモジュールをリビルドする時、コンパイラはモジュールをインポートする翻訳単位をビルドするために使用しているすべてのBMIの場所を認識している必要があります。できれば、この情報を何らかの形式で（他の情報と共に）記録しておくといい。例えば現在MSVCが`/sourceDependencies`に対して生成するのと同様の簡単に解析可能なjson等の形式で

例えば次のようなソースファイルに対しては
...
次のようなソース依存関係ファイル（source-dependencies）になります
...
`ModuleB.ixx.ifc`ファイルはグループBのビルドの一部としてビルドされており、その`source-dependencies.json`ファイルは同じ場所にあるはずです。したがって、`C:/Path/To/Outputs/x64/MyConfig/ModuleB.ixx.ifc.json`が存在し、次のような内容となります
...
`C:/Path/To/LibraryA/x64/SpecialConfig/ModuleA.ixx.ifc`および他の`ifc`ファイルの近くに、これらと同様のjsonファイルがある事で、`source.cpp`のビルドに使用したすべての依存関係（インクルードファイル、モジュール名およびそのソース）が判明したことになります。また、すべての依存関係の正確な場所も同時に把握することができます。

メインビルドが成功した後で任意のモジュール（モジュールAを含めて）をリビルドできるようにする（3.1のシナリオ）ためには、このような依存家計jsonファイルに加えて、コマンドラインオプションなどの場所を示すものではないオプション（`#define`など）が必要です。
...
モジュールAをグループBのビルドで使用するにはAのBMIの場所をBのビルド構成に外から与える必要があります。これはBMIの名前が常に定まっていないためで、パッケージマネージャが行うかユーザーが手動で行う（コンパイラの検索パスのどこかにBMIを配置する）ことで可能です。BMIとモジュールを対応付けるためには、BMI名とモジュール名の対応が事前に分かっている必要があります。これはユーザーが主導で対応を指示する、モジュール名をBMI名にエンコードするなどの方法で可能となります。
...
仮にモジュール名のBMI名へのエンコードに関して標準化したとしても、それを要求してしまうことは、モジュール名が変更されるたびにBMI名/場所のビルドオプションを変更しなければならなくなる可能性があります。これは、ユーザー/IDEの両方に負担をかけることになります。

しかし、上記の`d.json`ファイルがBMIの近くにあれば、ビルドにモジュール検索パスだけが指定されている時でも、このパスですべてお`d.json`ファイルを発見することができ、それらを読み込むことでモジュール名とBMIパスの対応を記録したビルド前モジュールマップを作成することができます。

この情報は、グループBのソースでインポートされているすべてのモジュールの依存関係を解決するのに十分であり、必要なライブラリがすべて同一PC上に存在している場合はすべてのライブラリモジュールを解決することができます。また、BMIやモジュールインターフェースの名前にモジュール名をエンコードする必要もなくなり、異なるコンパイラがモジュールをビルドするために必要なすべての情報を提供できます

これらの考察に基づくいてこの提案では、コンパイラは次のものを含む依存関係の情報を（上記のような）JSON形式で出力できるようにすることを提案しています

- モジュール名（モジュールをリビルドする場合
- コンパイルオプション（
- モジュールの依存関係

また、ビルド済みモジュールを配布するすべてのライブラリは、以下の情報を含めるようにする事も提案しています

- モジュールインターフェースソース
- 各BMIの近くにある依存関係jsonファイル（上記`d.json`のような

この提案では各JSONファイルの形式について、現在MSVCが`/sourceDependencies`に出力しているものと同様のものを使用していますが、実際のフィールド名や追加の内容については、標準化委員会における合意が必要です。
