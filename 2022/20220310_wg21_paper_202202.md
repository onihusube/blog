...
- CFPの宣言は同名のCPOを導入する
- 特定のCFPのカスタマイゼーションポイントにアダプトするためには、同名の関数を`override`指定子付きで関数を定義する
- 特定のCFP（CPO）に対するアダプトを明示するために、CFPのフルスコープ名による関数定義構文
...
この`contains()`をカスタマイズするためには、`override`指定とともに同じ名前（スコープ名）で関数を（例えば*Hidden friend*で）定義します
