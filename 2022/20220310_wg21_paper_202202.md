...
現在C++で利用できるメモリフェンス機能には[`std::atomic_thread_fence`](https://cpprefjp.github.io/reference/atomic/atomic_thread_fence.html)があります。これは特定の`std::atomic`オブジェクトへの読み書きの操作がそのフェンスを越えて前後しないようにするためのもので、フェンスが置かれているとそのスレッドの全ての`std::atomic`オブジェクト（あるいは間接的に他の変数）の読み書きの順序に影響を与え、結果的に同じ変数へのアクセスを共有する全てのスレッドに対して影響を及ぼします。

メモリモデルの説明によく見られる限定された2つのスレッド間でのメッセージパッシングの例のように、特定の少数のスレッドの間でのやり取りのみでフェンスの効果が必要である場合、フェンスのもたらす効果をその他多くのスレッドの間でも保証することは場合によってはとても高価になります。

メッセージフェンスとは、明示的に指定した特定のオブジェクトに対してのみフェンスの効果を適用することで、そのようなユースケースにおいてさらなる最適化を促そうとするものです。そのようなメッセージフェンスを標準化することで、ハードウェアや環境固有のより効率的なメカニズムを容易に利用できるようになり、非標準の機能（CUDAの`__shared__`など）を使用している場合においてもある程度の移植性を確保することができるようになります。
...
提案されているのは、`std::atomic_message_fence`と`std::atomic_object_fence`です。`std::atomic_thread_fence`はその存在が`std::atomic`オブジェクトへのアクセスに対してのみ影響及ぼし、他の変数へは直接的な影響を持ちません。`std::atomic_message_fence`はそれと同様に、そのフェンスが介在する`std::atomic`オブジェクトに対して暗黙にメッセージフェンスを適用するもので、`std::atomic_object_fence`は非`std::atomic`変数を指定してその変数に対してメッセージフェンスを適用するものです。

`std::atomic_message_fence`が提供する保証をメッセージフェンスと呼び、`std::atomic_object_fence`が提供する保証をオブジェクトフェンスと呼んでいます。

A : `O`に対する評価`A`と同じオブジェクト`O`に対する別の評価`B`は*happens before*関係を持つ  
B : もしそれらについて、フェンス`X`と`Y`が存在して、`A`が`X`よりも前に順序づけられ（*sequenced before*）  
C : `X`がオブジェクトフェンスならば`O`とともに起動され  
D : `Y`がオブジェクトフェンスならば`O`とともに起動され  
E : `Y`は`B`よりも前に順序づけられ（*sequenced before*）、かつ  
G : `X`と`Y`がスレッドフェンスである場合
F : `X`は`Y`に同期する（`X` *synchronize with* `Y`）

A if B, C, D, E, and F if G. -> B、C、D、Eの場合はA、Gの場合はF。 
