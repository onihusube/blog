...
`std::indirectly_comparable<T*, T*, Pred>`は`std::indirect_binary_predicate<Pred, projected<T*, identity>, projected<T*, identity>>`で定義されており、それは`std::projected<T*, identity>`型のイテレータ`it`に対する`*it`が有効な式であるかどうかをチェックしています。そこでは、`projected<T*, identity>`の関連するすべての型（`Holder<Incomplete>`も含む）に対して完全性を要求しており、`Holder<Incomplete>`の完全性が要求されてエラーが発生しています。
