...
展開に当たっては、対象のオブジェクトからエラー状態と中身の値を取り出す必要があり、また、それらの値から戻り値をどう構築するかを指定する必要があります。それを担っているのが`std::try_traits`とう型特性で、次の関数を持っています

- `is_ok` : オブジェクトのエラー状態を取得する
- `extract_value/extract_error` : 正常値/エラー値を取得する
- `from_value/from_error` : 正常値/エラー値から戻り値型を構築する
