...
- ディレクトリ順序の脆弱性
    ...
    - ヘッダ名がインポート可能としてリスト化されている場合、インクルードディレクトリの順序に基づいた探索を実行せず、明示的に定義されているヘッダユニット/BMIを使用する
...
攻撃者は、以前の関数呼び出しを行い同じスタックスロットが再利用されるようにすることで、`backlog`の値を制御できます。
...
[この記事のMarkdownソース](https://github.com/onihusube/blog/blob/master/2022/20221225_wg21_paper_202211.md)
