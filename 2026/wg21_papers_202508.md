...
C++26規格書のCommittee Draft。非委員会メンバは見られないようです。
...
- LIFO Pushアルゴリズムに関する歴史を纏めた"The Ancient History of LIFO Push"セクションを追加
...
- P3754R0に合わせて提案を更新
- Sofia会議でのフィードバックを適用
- UBリストからUBとして指定されるべきではなく、CWGIssugeがあるものを削除
...
- [P3428R1 Hazard Pointer Batches - WG21月次提案文書を眺める（2024年12月）]()
...
- [P3692R1 How to Avoid OOTA Without Really Trying - WG21月次提案文書を眺める（2025年07月）]()
...
- [P3739R1 Standard Library Hardening - using `std::optional` - WG21月次提案文書を眺める（2025年07月）]()
...
- LEWGレビューを反映
    - `std::nontype`は`std::constant_arg`にリネームする
...
この提案では その他の設計上の修正 のカテゴリに次の問題を追加しています
...
- `sender`非対応コルーチンは`task`を`co_await`できるべき
    - いくつかの制約（3.5.15 Sender Unaware Coroutines Should Be Able To co_await A task を参照）を考慮すると、現時点では`task`に`operator co_await`を追加しないことが妥当と思われる
...
- 右辺値修飾子の欠落
    ...
    - コルーチンはコピーできず、コルーチンハンドルを別のオブジェクトに転送する場合その操作は右辺値修飾するひつようがあるため、`task<T, E>::connect`も右辺値修飾する必要がある
...
これには`views::join`を通すことによってもっとも強くても`bidirectionl_range`にしかならないなどの問題があり、専用のアダプタを用意することで直観的に使用できるようになるとして、`views::cycle`を追加することを提案しています。
