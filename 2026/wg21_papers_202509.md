...
- [P3639R0 The `_BitInt` Debate - WG21月次提案文書を眺める（2023年12月）](https://onihusube.haablog.com/entry/2025/09/30/224815#P3639R0-The-_BitInt-Debate)
...
2. 関数デコレータ
    - `pre/psot`のアサーションは関数呼び出しの前後で処理を実行する専用の構文になっている
    - これはpythonではデコレータとしてより汎用的名機能になっており有用であるため、このデコレータ機能によって`pre/psot`のアサーションを実現する
