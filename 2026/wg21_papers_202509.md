...
`std::uncaught_exceptions()`から`constexpr`指定を削除する提案。
...
- `std::uncaught_exceptions()`から`constexpr`指定を削除する
    - `std::current_exceptions()`はノータッチ
- 定数式で使用できる専用の`std::consteval_uncaught_exceptions()`を追加する

この提案の場合、`std::uncaught_exceptions()`は定数式で使用できなくなり、定数式で使用するには名前の違う`std::consteval_uncaught_exceptions()`を使わなければなりません。これによって、実行時とコンパイル時でコードを共通する場合は`if consteval`による分岐が必要になります。また、`std::current_exceptions()`については言及されていません。

P3818R1のところでも述べたように、結局どちらの解決策もC++26のNBコメント解決フェーズに導入するには適さないとして、P3824R0で`std::uncaught_exceptions()`と`std::current_exceptions()`から単に`constexpr`を削除することをC++26向けの修正とするようです。
