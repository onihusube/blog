...
例えば数値型の最大値や最小値等、数値型の満たしている各種特性を取得するのに現在は`std::numeric_limits`が用意されています。これは少なくとも`<type_traits>`ヘッダにあるような型特性が見出されるよりも以前から存在しており、その利用法やユーザー定義型に対する適合法はあまり現代のC++的ではありません。

ユーザー定義型に対する特殊化を追加する場合、ジェネリックな利用のために本来必要のない数値特性についてもそれっぽい値を返すように実装するひつようがあります。あるいは、ある数値特性を提供しているのかどうかを知る方法が提供されていません。  
...
template <template <class> class Trait, class T, class R = T> inline constexpr R value_or(R def = R()) noexcept;
```
...
constexpr R value_or( R def = R() ) noexcept {
  if constexpr(value_exists<Trait, T>)
...
このように、数値型に対する数値特性が個別に分かれていることによって、数値型によっては必要のない数値特性は提供されず、新しい数値特性を追加する際に既存のユーザー定義特殊化を壊してしまう事もありません。ユーザーが特殊化を追加する際も必要な数値特性についてだけ特殊化を行えばよくなります。
