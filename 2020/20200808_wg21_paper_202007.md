...
ビルドシステムなどの外部ツールが、そのターゲットとなる環境を決めるための名前の列である*Target Tuplets*についての基礎的な定義を標準に追加する提案。

*Target Tuplets*は環境を1つに定めるのに必要となる次の4つの情報の列として規定されます。

- *Architecture*
    - CPUの命令セット（ISA）
    - `x86_64, armv6`など
- *Vendor*
    - C++ツールチェーンを提供する事業体名（組織、個人、PC）
    - `apple, nvidia`など
- *Operating system*
    - `linux, win32`など
- *Environment*
