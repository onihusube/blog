...
このリビジョンでは、1引数の`​defer, dispatch​, ​post`がその引数の`CompletionToken`から（色々やって）型`::executor_type`を取得できることを要求し、それが同様に（色々やって）取得できる`get_executor()`の戻り値型と一致することを要求するようになった事などです。
