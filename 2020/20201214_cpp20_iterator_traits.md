...
その結果、`I`の`iterator_category`を見ればC++17までのイテレータとしてのカテゴリが、`iterator_concept`を見ればC++20からのイテレータとしてのカテゴリがそれぞれ取得できることになります。
...
C++17のイテレータをイテレータコンセプトに渡したときを考えてみましょう。

`ITER_CONCEPT`を通してイテレータカテゴリが`I::iterator_cateogry`から取得され、コンセプトによってそのインターフェースがチェックされます。C++17イテレータとして定義されていて、C++20での要件も満たしている場合は問題なくそのカテゴリのC++20イテレータとして判定されるでしょう。  
...
こうしてみると古いものからは古いものとして、新しいものからは新しいものとして見える、というなんだか当たり前の話に見えてきますね・・・
...
（本当は前回の最後に載せるべきでしたが忘れていたのでここに書いておきます・・・）
