...
ここで、2つのスタイルの例にて使用されている`filter`や`transform`は異なるオーバーロードです（例えば、`filter(Rng&&, Pred&&)`と`filter(Pred&&)`）。パイプラインスタイルでは`|`演算子の呼び出し前にそこに渡す為だけのラッパを生成するだけの処理になりますが、第一引数に`range`オブジェクトが渡っている最初の例では直接`view`オブジェクトが返されます。
