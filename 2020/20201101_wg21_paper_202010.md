...
*decay-copy*というのは関数テンプレートに引数を渡すときに行われるキャストのことです。関数テンプレートのテンプレートパラメータによる引数に値を渡すとき、左辺値は右辺値に、配列はポインタに、CV修飾を除去しつつ変換されます。
...
- [P0958 進行状況](https://github.com/cplusplus/papers/issues/293)
