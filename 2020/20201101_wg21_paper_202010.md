...
一方、C++ Executorライブラリが非同期並行処理のための基盤となるものであることを考えると、どこかから渡されてきた*scheduler*を*executor*として扱うことも避けるべきです。これは不可逆変換ではありませんが、広い契約を持つ関数の中で狭い契約を持つ関数が呼ばれているようなもので、広い契約を期待する呼び出し元の期待は満たされません。  
...
標準ライブラリのIsuueのうち2020年11月のオンラインとうひょうにかけられるもののリスト。
