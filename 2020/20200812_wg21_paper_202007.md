...
前回の記事を参照
[https://onihusube.hatenablog.com/entry/2020/06/01/001003#P2128R1--Multidimensional-subscript-operator]
...
EWGでのレビューを受けて、不完全だった提案の文言をいったん削除し、代わりに著者の型が重視している側面について追記されています。
...
グローバルモジュールフラグメントの破棄されなかったものが、テンプレートのインスタンス化時に参照されないようにする提案。
...
export void f(C&& container) {
  auto it = std::begin(container);  // 依存名なのでここでは参照されているとみなされない
}
...
これによって、グローバルモジュールフラグメントは完全にモジュール内部で完結するものになり、モジュール外部からは気にしなくてもよくなります。実装はモジュールのコンパイル後にグローバルモジュールフラグメント内のものを残すかどうかを自由に選択できるようになります。
...
#### モジュールパーティションのインスタンス化経路

モジュールインターフェース単位`M`で定義されているテンプレートのインスタンス化地点が`M`（のインターフェース単位）内に無い場合、インスタンス化経路としてプライマリインターフェース単位の終わりの地点（プライベートモジュールフラグメントの前）が設定されます。

これは、テンプレートがインターフェースパーティションで定義され、実装パーティションからインスタンス化されている場合を考慮していません。実装パーティションからはそのプライマリモジュールインターフェース単位の終わりの地点は必ずしも利用可能ではありません。

#### モジュールパーティション内のグローバルモジュールフラグメント

モジュールは複数のパーティションを持つことができ、インターフェースパーティションは少なくともプライマリインターフェース単位からエクスポートされなければなりません。パーティションも当然グローバルモジュールフラグメントを持つことができ、その場合モジュール内に複数のグローバルモジュールフラグメントが存在することになります。パーティションへの分割は外部からは観測不可能にならなければならないため、それらはプライマリインターフェース単位にて統合されることになるでしょう。

その際には2つの選択肢があります。

- グローバルモジュールフラグメントの和集合がインポートした側から到達可能となる
- グローバルモジュールフラグメントの共通部分がインポートした側から到達可能となる
