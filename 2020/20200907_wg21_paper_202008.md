...
[https://onihusube.hatenablog.com/entry/2020/07/05/003248#P1949R4--C-Identifier-Syntax-using-Unicode-Standard-Annex-31]
...
以前の記事を参照
[https://onihusube.hatenablog.com/entry/2020/05/01/194425#P2013R1--Freestanding-Language-Optional-operator-new]
...
虚数単位には`i`が使用され、複素数の各パートの数値型`T`のフォーマットは再帰的に`T`のフォーマット指定（`std::formatter<T>`）に委譲されます。虚数部が`0.0`の時、虚数部を消して出力するのか`0.0`を出力するのかはさらなる議論を必要としているようです。
