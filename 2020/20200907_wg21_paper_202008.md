...
なぜなら、`views::split`すると得られるこの`v`は`forward_range`（*forward iterator*で構成された範囲オブジェクト）なので、`data()`や`size()`などというメンバ関数はなく、そのイテレータはポインタですらありません。ついでに言うとその`v`は`common_range`（`begin()`と`end()`の型が同じ範囲）ではありません。
