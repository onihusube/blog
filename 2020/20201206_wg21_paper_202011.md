...
`std::indirect_value<T>`はテンプレートパラメータでコピーをどうするかとカスタムデリータを指定することができます。デフォルトのコピーは新しいメモリ領域にコピー構築を行い、そのポインタを保持するものです。ムーブも定義されており、それは`unique_ptr`同様所有権を移動するものです。
