...
先程も出て来た`tag_invoke`はCPOの行う呼び出しのうち、CPOと同名の関数をADLによって探索する部分を置き換えるためのものです。
...
struct my_executor {
  template<typename F>
  friend void executor(const my_executor&, F&& f) {
    // オリジナルのexecutorの処理、略
  }
};
```
...
struct my_executor {
  template<typename F>
  friend void tag_invoke(std::tag_t<std::execution::execute>, const my_executor&, F&& f) {
    // オリジナルのexecutorの処理、略
  }
};
```

ここでの`friend`関数は*Hidden friends*というイディオムであり、同じ名前空間のフリー関数として定義していても構いません。しかしどちらにせよ、そこの関数名にCPO名ではなく`tag_invoke`という名前を使うようになることで、CPOをいくら増やしてもユーザーコードではそれを気にせずに同じ名前を使用することができるようになります。
