...
値カテゴリの違いによるオーバーロードはdeducing thisでまとめよう
...
class optional {
  ...
  // 引数先頭でthisに続いて引数を宣言すると、*thisを明示的に受け取ることができる
  // this引数をテンプレートで受け取ることもできる
  ...
  // これにより、最大4つのオーバロードを1つにまとめられる！
  template<typename Self>
  decltype(auto) operator*(this Self&& self) {
    // forward_like()は、thisの状態によってメンバ変数のconst/参照修飾を調節する
    return std::forward_like<Self>(self.value_);
  }
