...
constexpr auto enum_to_string(E value) -> std::string {
  constexpr auto enumerators
    = std::meta::enumerators_of(^E) // Eの列挙子のリフレクションを取得
    | std::views::transform([](std::meta::info e){
        return std::pair<E, std::string>(
          std::meta::extract<E>(e), // リフレクションeからEの値を取得
          std::meta::name_of(e));   // リフレクションeからその宣言名を文字列で取得
      })
    | std::ranges::to<std::vector>(); // vectorへ詰め替え
