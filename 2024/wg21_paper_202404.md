...
// RGB画像を参照するmdspanエイリアス
using rgbimg = std::mdspan<rgb_pixel, std::dextents<std::size_t, 2>>;

// メモリ上にロードされた画像データ
auto [image_byte, width, height] = loadimage("img.bmp");

// 画像データを2次元行列として参照
rgbimg image{image_byte.data(), width, height};

using namespace std::views;

// 全画素へのアクセス
for (auto [y, x] : cartesian_product(iota(0uz, height), iota(0uz, width))) {
  // 読み取り
  const auto [r, g, b] = image[y, x];

  // 書き込み
  image[y, x] = {r/2, g/2, b/2};
}

template<typename T>
using mat33 = std::mdspan<T, std::extents<std::size_t, 3, 3>>;

auto mat_add(mat33<int> m1, mat33<const int> m2) {
  using namespace std::views;

  for (auto [y, x] : cartesian_product(iota(0, 3), iota(0, 3))) {
    m1[y, x] += m2[y, x]; // 多次元添え字演算子オーバーロード
  }
