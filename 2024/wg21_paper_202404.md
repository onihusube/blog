...
```cpp
std::vector<std::vector<double>> mat33{};

// 3x3行列を初期化
for (int y = 0; y < 3; ++y) {
  auto& row = mat33.push_back();
  row.resize(3, 0.0);
}
...
```cpp
// 3x3行列を初期化
std::vector<double> data(3*3, 0.0);

// 静的3x3行列
using mat33_t = mdspan<double, extents<size_t, 3, 3>>;
// 動的3x3行列
using mat2d_t = mdspan<double, dextents<size_t, 2>>;

// mdspan初期化
mat33_t mat33s{data.data()};
mat33_t mat33d{data.data(), 3, 3};
