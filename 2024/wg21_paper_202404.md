...
std::barrier<> sync{thread_num};

// データ並列処理の1単位
void kernel(uint64_t in_idx) {
  // 処理対象のデータのロード、非アトミックでok
  float local_data[5];
  local_data[0] = data[in_idx-2];
  local_data[1] = data[in_idx-1];
  ...

  // メインの処理、元配列にアクセスしない
  const auto [out_idx, result] = compute(in_idx, local_data);

  sync.arrive_and_wait();

  // 結果の出力ここだけアトミックである必要がある
  std::atomic_ref out{data[out_idx]};
  out += result;  // 他スレッドと被っても安全
}
```