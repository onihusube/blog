...
提案では、標準ライブラリ中で`explicit`を使用するパターンを3つに類別しています。
...
3. みだりな構築を回避するための、タグ型における引数無しコンストラクタの`explicit`
    - タグ型を`{}`から構築させないようにし、構築の差異はその型名を明示的に書かせる
    - 例えば`std::unexpect_t`はそれを受け取る場所（`std::expected`コンストラクタ）で`{}`を使用できず、`std::unexpect_t{}`もしくは`unexpect`の使用を強制する

`<ranges>`の一部を除いて、この3つのパターンに反する`explicit`の使用は見つからず、これは現在の標準ライブラリが暗黙の一貫性に従って`explicit`を使用していることを示しており、この一貫性を言語化することで1つのポリシーとなり得ます。
