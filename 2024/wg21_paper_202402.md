...
3. それが適切な場合には、`sender`にょって形作られた作業を即座に実行（eager execution）する
...
Allocator awareにしようとする場合、構築時に渡されたアロケータを`inplace_vector`は憶えておく必要があります。その受け取り方にはいくつかの方法があり、ここでは4種類の方法を提案しています
