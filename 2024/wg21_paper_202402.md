...
関数は何度でも再宣言することができ、その都度仮引数名を変えることができます。そのため、どの宣言の仮引数名を取得すべきかが不透明であり、何らかの保証が無い場合にコンパイル時の仮引数名の取得の使用は危険なものになりかねません。
...
|一貫性した命名の強制|yes|yes|partially|yes|yes|
...
- 関数仮引数の型のリフレクション
- 「一貫性した命名の強制」ソリューションに基づく関数仮引数名のリフレクション
