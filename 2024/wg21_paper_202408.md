...
### [P3050R2 Fi❌C++26 by optimizing linalg::conjugated for noncomple❌value types](https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2024/p3050r2.html)
...
この文書では、``std::execution`` フレームワークにおける**非同期操作の入れ子状態のライフタイム**について、いくつかの設計上の選択肢を検討し、推奨案を提示しています。

非同期操作において、入れ子状態のライフタイムは、現在**最長の状態を維持**するように仕様化されています。  これはストレージの最大消費、一見予期しないアクセスによる動作の明確化、リソース管理の長期化といった影響をもたらします。

この文書では、**入れ子操作の状態をレシーバーが破棄することを義務付ける**最小限のアプローチ、**操作が生成された値を使い終えたらすぐに先行操作の状態を破棄する**アドホックなアプローチ、**実装依存**のアプローチといった代替案が検討されています。

**各アプローチにはメリットとデメリット**があり、例えば最小限のアプローチはストレージ消費を最小限に抑えますが、未定義の動作が増加する可能性があります。 アドホックなアプローチは、同期コードとのアナロジーを維持しようとしますが、複雑さを増す可能性があります。 実装依存のアプローチは、移植性の問題を引き起こす可能性があります。

文書では、**最小限のアプローチを支持**する議論が展開されています。 これは、プログラマーに明示的なライフタイム管理を強制することで、より堅牢なコードにつながると主張しています。 また、標準アルゴリズムと非標準アルゴリズムとの間で一貫性のない動作を防ぐことができるとも述べています。

最終的には、**``std::execution`` の設計者は、入れ子状態のライフタイムについて明確な決定を下し、標準アルゴリズムで一貫した動作を保証する必要がある**と結論付けています。 
