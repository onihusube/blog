...
C++26からはこれが緩和されてErroneous Behaviourとなり、おそらく0で初期化されるようになります。
...
C++26以降にこれまで通りの未初期化領域が欲しい場合は、`[[indeterminate]]`属性を指定することで挙動を維持することができます。
...
EB環境の下ではこのように、初期化されておらず不定値を持つ変数を明示することを強制します。

C++では不明な属性は無視するため`[[indeterminate]]`属性そのものは今日から使用し始めることができます。今すぐC++26に移行しないコードでも、未初期化変数を目立たせる目的で使用しておくと、将来すんなりとC++26に移行することができるでしょう（この部分に関してだけは）。
