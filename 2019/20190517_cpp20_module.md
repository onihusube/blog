...
- 可視
  - ある宣言は（いずれかの）名前探索において見つかる（候補に上がる）時、その翻訳単位において可視となる
- 到達可能
  - ある宣言は、（名前探索とは無関係に）その宣言の持つ意味論的な性質が利用可能である時、その翻訳単位に置いて到達可能となる
...
例えば、モジュールをインポートするとそのインターフェース内でエクスポートされている宣言はインポートした側で可視となり、同時に到達可能になります。  
しかし、そのインターフェース内でエクスポートされていない宣言は到達可能ではありますが、可視ではありません。  
このために、上のインターフェースオンリーなモジュールの項で上げたような意図しないODR違反が起こってしまう可能性があります。

#### 到達可能性とODR（One-definition rule）

モジュールは1つ以上の翻訳単位からなり、インポートによってそのインターフェースを別の翻訳単位に取り込むことができます。その際、これまでのODRを適用しようとすると少し不便になってしまいます。そのため、到達可能という概念を利用してODRに少し変更が加えられます。

これまでのODRでは、ある宣言が複数の定義を持つことを基本的に禁止していました。それは翻訳単位を超えたとしても同様で、最終的なリンカの処理終了後には宣言に対する定義はただ一つにならなければなりません。  
C++20以降のODRは、ある宣言に対して複数の定義がプログラム内に存在したとしても、ある定義（をした所）から見た時に以前の定義が __到達可能__ でなければ、同じ宣言は複数の定義を持つことができます。
