...
翻訳単位`Q`が別の翻訳単位`R`にインポートされている時、`Q`の名前空間スコープで宣言された名前`X`の潜在的なスコープは次の全てを満たす場合に
`Q`を（直接または間接的に）インポートする`R`の最初のmodule-import-declarationかmodule-declarationに続く、`R`の対応する名前空間スコープを含むように拡張される。
