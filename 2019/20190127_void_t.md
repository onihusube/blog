...
detection idiomとは`void_t`を利用して、ある型が持つ特性や適用可能な操作をコンパイル時に検出する手法です。
...
`void_t`を主に利用しているのは`std::true_type`を継承する方の`detect`で、チェックする一変数メタ関数（Check<T>）を実行しその戻り値型をvoidへ写しています。
```cpp
template<class T, template<class> class Check>
struct detect<T, Check, std::void_t<Check<T>>> : std::true_type {};
```
...
```
この時、引数は2つしか渡していないから部分特殊化を選択、とはなりません。プライマリテンプレートは3引数で宣言されているので3つ目の引数をまず充填します。どこからというと、プライマリテンプレートのデフォルト引数（すなわち`void`）を入れます。
