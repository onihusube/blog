...
すると残った所はde Bruijn sequenceと`pop_lsb`のかけ算です。普通の数値のかけ算なら結果がどうなるかを考えるのは少し難しいですが、`pop_lsb`はどこか1ビットだけが立った値です。  
つまり、その値は必ず`2^n`の値になります。その2のべき乗数値との掛け算はすなわち`n`ビット左シフトに相当します。
