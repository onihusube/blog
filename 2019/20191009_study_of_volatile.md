...
- `volatile`オブジェクトへのアクセス（読み/書き）の順序を保証し
  ...
  - CPUでのアウトオブオーダー実行時にもアクセス順序が保証される（例えば、適切なメモリバリアが挿入される）
    - 適合実装にはコンパイラだけでなく実行環境も含まれており、適合実装は少なくとも抽象機械の観測可能な動作（ここでは`volatile`オブジェクトへのアクセス）をエミュレートする責任があるため
- `volatile`オブジェクトへのアクセスに関してはコンパイラの最適化対象外となる
...
ただ、他のプログラミング言語の中には`volatile`指定の効果に、C++の`volatile`の効果に加えて`std::atomic`の持つ効果がくっついている事があります。例えばC#やJavaがそうですが、このような言語においてはその規則の範囲内において期待する効果が得られるかもしれません。  
...
そして、CPUのデータバス幅未満のサイズの変数でアライメント要求などを満たしていれば、その読み書きは必然的にatomicに行われるため、CPUと型のサイズの組み合わせによっては変な値を読みだしてしまう事が起きないかもしれません。
