...
私もそう思うが、もちろんこれは不可能だ。ユーザーは古いバージョンをサポートするコンパイラを要求するため、非推奨の機能でさえ実際には機能しなかった。主要な機能を廃止することは不可能であり、小さな機能を廃止することは大きな利益のない面倒なことだ。
20年前のC++コードは今日でも実行できる、これはC++を利用する大きな理由である。その理由の1つは、今日書いたコードは20年後にも動作するという機能を抱かせるためだ。
