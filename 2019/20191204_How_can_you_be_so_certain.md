...
C++標準化委員会における「委員会による設計（より正確には委員会連合による設計）」プロセスでは、一般に表明されたすべてのニーズの和集合を包含するような設計に落ち着く傾向にある。  
そのように設計された機能の承認当初のものは酷いもので、肥大化している。表明されたすべてのニーズはほとんどのC++プログラマにとって（直接・間接的にも）現実的で需要なものではないためである。
