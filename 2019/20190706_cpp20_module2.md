...
そして、これをテンプレートの定義点から順番に並べたものがインスタンス化経路となります（孤立点が発生し得ますが・・・）。
...
struct Wrap {
  ...
  /*このコピーコンストラクタが暗黙定義される時、Dのコピーコンストラクタが
  */
}
...
int main() {
  ...
  Wrap w1{100};  //ok、集成体初期化
  Wrap w2 = w1;  //コピーコンストラクタのodr-used
}
```
