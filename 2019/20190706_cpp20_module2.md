...
インポートする翻訳単位からエクスポートされている宣言を抽出するには、`export`宣言の導入する名前が決定していなければなりません。そして、そのような名前を決定するためには、少なくともプリプロセスが終了している必要があります。

従って、モジュールは少なくとも翻訳フェーズ4（プリプロセッサの実行とプリプロセッサディレクティブの削除）完了まではビルドされているはずです。すなわち、モジュールのインポート側から見ればモジュール内部ではプリプロセッサ実行は完了しているはずです。

また、モジュールはテンプレートをエクスポートできます。エクスポートされたテンプレートはモジュール外部の任意の場所でインスタンス化されるので、モジュール内部のテンプレートはほぼコンパイルされていないはずです。  
そのため、モジュールはどれだけビルドしても翻訳フェーズ8（テンプレートのインスタンス化）まで到達することはないはずです。

これらの事から、モジュールはビルドにおいて翻訳フェーズ4～7（4完了～7完了）のどこかの段階まではビルドされていると推測できます。
...
1. `void f(T t, U u)`（moduleA.cpp内、定義）
2. モジュールBの末尾
...
それに加えて、（メンバ）関数テンプレート及びクラステンプレートの（静的）メンバ関数は、次のどちらかをもう一つのインスタンス化地点として持ちます
...
1. あるテンプレート`T1`のインスタンス化地点がそれを囲むテンプレート`T2`の中にあり、`T2`のインスタンス化に伴って`T1`がインスタンス化されるときのインスタンス化コンテキストは以下の両方を含む
    - `T2`のインスタンス化コンテキスト
...
4. 上記に当てはまらないテンプレートのインスタンス化コンテキストは、そのテンプレートのインスタンス化地点を含む
...
ただし、3番目の地点（main.cpp内）からは`M1`のインポートを通じて`S`の定義は推移的にインポートされており、これに到達可能となるかは実装依存です。
...
前項テンプレートのインスタンス化経路（インスタンス化コンテキスト）のルールは、ADLにおける探索時に可視となる名前についてにも関わってきます。

ADLがある名前空間`N`を探索する時、あるモジュール`M`内の宣言は次の場合に可視となります
- テンプレート引数に依存する名前の探索時、`N`内の宣言`D`がインスタンス化コンテキスト内の任意の点の修飾名探索において可視であるとき、次の場合を除いて`N`内の宣言は可視となる
  - `D`は別の翻訳単位でも宣言されていて、グローバルモジュールに属しており
    - 破棄されている、か
    - 内部リンケージを持つ
- `M`のインターフェース内の`N`の内側にある宣言`D`は、`D`を囲む最も内側の非inline名前空間に`M`に属する関連エンティティがある場合に、すべて可視となる
