...
これを`mdspan`に組み込みます。`mdspan`のテンプレートパラメータは`mdspan<T, E, LP, AP>`の順で、それぞれ`T`が要素型、`E`がエクステント型（`std::extent<I, ...>`）、`LP`がレイアウトポリシー型、`AP`がアクセサポリシー型、となっています。今回`AP`は弄らず`LP`を変えたいためそこまでの3つのテンプレートパラメータの手動指定が必要です。

`mdspan`では少なくともエクステント型を都度指定する必要があるので、`mdspan`を利用する際はあらかじめ必要なテンプレートパラメータを埋めた型エイリアスを作成しておくと便利です。
