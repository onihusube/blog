...
明示的オブジェクトパラメータとは、C++23で導入されたDeducing `this`という機能のことで、`this`に相当する引数を明示的に記述してメンバ関数を宣言できる構文のことです。
...
こうして宣言した関数は、普通に使用する分にはメンバ関数のように使用できますが、規格的にはどちらかというと`static`メンバ関数のような扱いをされています。特に、そのアドレスはメンバ関数ポインタではなく普通の関数ポインタとして取得され、メンバポインタ特有の少し変わった関数呼び出しではなく通常の関数ポインタの用法によって呼び出しができます。
...
この例の#2はどちらの`f`の宣言ともそぐわないので候補が見つからずにill-formedとなりますが、#1がどうなるのかは規定されていないようです。

この問題はコア言語のIssueとして報告され、この提案はその解決のためのものです。
...
1. #1 #2及び#4 #5はそれぞれ同じ振る舞いをし、#3 #6はill-formed
...
EWGにおける議論の結果、オプション2が選択されたようです。これによって`(&function-id-expression)(expr-list)`という式では、ベストマッチする候補を選択する前に、オーバーロード候補集合をその要素のポインタに減衰させてから、オーバーロード解決を行うようになります。
...
struct C {
  ...
  void d() {
    c();               // error: #2と#3の間で曖昧
    (C::c)();          // error: 同様
    (&(C::c))();       // error: オーバーロード候補（this->C::c）のアドレスを解決できない
    (&C::c)(C{});      // #1を選択
    (&C::c)(*this);    // error: #2が選択されるがill-formed
    (&C::c)();         // #3を選択
  }
};
```
