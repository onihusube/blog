...
void A::g() {
  ...
  // C++20以前からの振る舞い（fの宣言を void f()のようにしたとして）
...
この例の#1は適切な候補が見つからないためエラーとなることがわかりますが、#2がどうなるのかは規定されていないようです。
...
ただしこの扱いは、関数ポインタから呼び出した時には行われず、`(&A::f)(A())`は`A::f`がオーバーロードされていない場合にのみ適切に呼び出すことができるはずです。ただし、現在の規定ではそれすらも曖昧となっています。
...
void A::call() {
  ...
  g(A{});       // ok static
  g();          // ok 非static
  (&A::g)(A{}); // ng、オーバーロードされておりアドレス取得対象が確定しない
