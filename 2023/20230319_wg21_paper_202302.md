...
ただし、各次元のextentに1つでも動的なもの（`std::dynamic_extent`）を含む場合、それをデフォルト構築するとインデックス空間が空になるため（？）問題とならず、デフォルトコンストラクタで問題があるのはすべてのextentが静的に定まっている場合のみです（と書いてありますが問題になる気がします・・・）。
