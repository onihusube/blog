...
ファイル`2mb-image.png`が2MBのサイズのファイルだとすると、このコードはまず2MBの暗黙配列を作成します。それによって関数のスタックが2MB消費されることになります。ファイルサイズが大きければ、簡単にスタックオーバーフローを起こすことができます。しかも、この問題はコード上から隠されており、`std::initializer_list`の振る舞いをよく知らないと気づくことが困難です。
