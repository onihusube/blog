...
1. ストレージ容量の削減
    - メンバ関数とその対象オブジェクトをバインドする場合、その結果オブジェクトは元のオブジェクトよりも大きくなり、`std::function`等のSSOのサイズを超えてしまう
    - 呼び出したいメンバ関数等はコンパイル時にわかっていることがほとんどだが、これは回避できない
    - NTTPとしてメンバポインタを転送することで、そのストレージサイズを削減できうる
2. デバッグ情報のスリム化
    - 中間層削減?
