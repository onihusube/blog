...
void check(int const (&param)[3]) {
  ...
  constexpr auto s2 = array_size_p(param);  // ok
}
```
...
このリビジョンでの変更は、*r-char-sequence*（生文字列リテラルの文字列）にユニバーサル文字名が含まれていない事を明確にし事と、文字列リテラルオブジェクトが翻訳フェーズ5で初期化済みと取れる文章の削除、提案する文言の修正などです。
