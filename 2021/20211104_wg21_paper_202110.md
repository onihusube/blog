...
int main() {
  foo({1, 2, 3}); // C++17まではOK、C++20からはerror
}
```
...
また、`std::initializer_list`コンストラクタは`{}`初期化時に最優先で選択されることから、この変更は破壊的変更となります。しかし、`std::initializer_list`コンストラクタに制約を設けることでこの影響は低減でき、破壊的となるのは非常にレアケースとなりそうです。
