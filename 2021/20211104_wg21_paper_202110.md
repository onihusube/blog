...
クラス型のオブジェクトとそのメンバについて、`const`性も含めた完全転送を行う`std::forward_like`の提案。
...
struct wrap {
  ...
  auto value(this Self&& self) -> /*戻り値型は？*/ {
...
この例のように、Deducing thisによってキャプチャしているラムダの再帰を行う際に`forward_like`は特に重要となります。
