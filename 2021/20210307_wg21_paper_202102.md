...
また、`ranges::fold`のファミリとして、`foldr`に対応する`ranges::fold_right`と、範囲の最初の要素を初項として使用する`ranges::fold`である`ranges::fold_first`、最後の要素を初項として使用する`ranges::fold_right`である`ranges::fold_right_last`を同時に提案しています。
