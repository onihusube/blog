...
問題となったのは戻り値型の`decltype((expression))`です。これによる推論は参照のセマンティクスをデフォルトとし、左辺値の`return`に対して左辺値参照を推論します。一方、通常のラムダ式で戻り値型指定を省略した場合は値のセマンティクスがデフォルトであり、`decltype((expression))`の結果型を`decay`した型が推論されます。
...
この問題への対処のためにこの提案では2つの記法を提案しています。
...
こちらでは、提案1による最小式の戻り値型をデフォルトで値のセマンティクスとして、参照セマンティクスとするには追加の記法を強いるものです。
...
`()`を使用して参照を取得することは、`decltype`や`return`ですでに確立されています。
