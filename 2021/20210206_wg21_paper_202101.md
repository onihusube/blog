...
この提案では`std::ranges::cbegin/cend`の効果に引数の*range*オブジェクトから得られるイテレータ/センチネルのペアをラップして`const_iterator`化する効果を追加することで、`std::ranges::cbegin/cend`が常に`const_iterator`を返すように変更し、それを用いて`views::const_range`アダプタを追加することを提案しています。
