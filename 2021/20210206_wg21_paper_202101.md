...
定数式ではあらゆる未定義動作が許可されないため、コンパイラはすべての参照が有効であることを確認する必要があります。
...
ここでの`array_size`の呼び出しでは`param`のコピーが発生しています。定数式中で関数にコピー渡しする場合、コピー元も定数式でなければなりませんが、関数の引数は定数式ではありません。ローカルの配列（のポインタ）の場合は、定数式中でその関数が呼ばれていればそのローカル配列への参照（ポインタ）が有効であることが分かるので問題にはなりません。
