...
この提案はBoost Accumulators Libraryを参考にしており、これらの統計処理関数は`std`名前空間のグローバル関数としても、*Accumulator Object*というオンライン計算用クラスとしても提供されます（*Accumulator Object*では重み付き処理がサポートされます）。
