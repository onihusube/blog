...
- [P2093R0 Formatted output - ［C++］WG21月次提案文書を眺める（2020年6月）](https://onihusube.hatenablog.com/entry/2020/06/01/001003#P2168R0--generator-A-Synchronous-Coroutine-Generator-Compatible-With-Ranges)
...
![](./20210206_wg21_paper_202101/P2279R0.png)
...
明らかにMSVCの動作が望ましいのですが、`adaptor(args...)`の様に呼ばれたときに`adaptor`が`args...`をどのようにキャプチャするのかは規定がありません。また、`adaptor(range, args...)`と`adaptor(args...)(range)`が同じ振る舞いをするという要件は、ここで`args...`をコピーするという選択を排除している可能性があります。
...
それによって、上記のMSVC実装が合法かつ推奨する実装となります。これは、*range adaptor object*に渡された引数が参照によって束縛されず常にコピーによって束縛され、パイプラインに渡す時にはその値カテゴリに応じて束縛されたものを適切にコピー/ムーブする、という事でもあります。
...
これはMSVC STLにおいて`std::vector`を`constexpr`化する過程で発覚し、既に実装されているようです。
