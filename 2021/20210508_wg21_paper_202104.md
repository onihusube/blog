...
RCUを使用して、複数のリーダー（*reader*）が並行して存在する時に、共有データの更新を実行する例。リーダーは1つのスレッドで実行され、更新処理はそれとは別のスレッドで実行されます。


`print_name()`の`rcu_reader`インスタンスは、リーダーの処理が完了するまで参照しているオブジェクトが、`rcu_retire()`によって削除されないように保護しています。
