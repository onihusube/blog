...
に、先ほど得た内容（`T1 = int, T2 = T`と`typename T2 = typename T`）をフィードバックします。
...
が得られます。ここで注意点は、元のエイリアステンプレートのパラメータ（`auto N`）を勝手に消すことはせず、これも含めてフィードバックします（どうやらその際、エイリアステンプレートのパラメータを先頭に持ってくる様子？）。
