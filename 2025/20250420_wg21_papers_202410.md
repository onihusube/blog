...
このリビジョンでの変更は、名前を`cache_latest`に変更したことです。
...
トークンの`.try_associate()`に対してネストする`sender`を渡すことでスコープと`sender`を関連付け、その関連付け状態を表す`async_scope_association`オブジェクトを取得します。`async_scope_association`オブジェクトは`bool`変換によってその関連付けの状態を取得することができて、`true`を返す場合に“engaged”すなわち関連付けが有効であることを表します。
