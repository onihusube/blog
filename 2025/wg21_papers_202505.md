...
基本的には全ての並列アルゴリズムがこれに従っていますが少し変わった結果を返すアルゴリズムがあり、それが`ranges::rotate_copy`と`ranges::reverse_copy`です。これらでは、非並行版と同じ戻り値型を使用してこの原則に従うことで、その他の出力を行うタイプの並行アルゴリズムとは一貫性がありますが、少し問題があります。
...
現在の非並列版では`in_out_result`という2要素の型を使用しており、メンバ変数名もその数も異なるため、`auto`で受けて使用していたり構造化束縛していたりしてもエラーになります。
