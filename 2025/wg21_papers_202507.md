...
この提案は既存の標準文言の調整を行う変更のみになり、次の事を提案しています
...
P3074R7以前は、`X<make()>`のNTTPはアクティブメンバを持たない共用体オブジェクトだったのでNTTPとして有効でした。しかし、P3074R7の変更によってこの`u`は最初のメンバの生存期間暗黙的的に開始されることによってアクティブメンバを持つため、NTTPとして使用できずコンパイルエラーとなります。
...
1つ目の問題についてはP3074R7の変更をrevertし、トリビアルな共用体のimplicit-lifetime typeメンバの生存期間が自動開始されるポイントをplacment newまで遅延します。
